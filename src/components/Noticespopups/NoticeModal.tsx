        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md">
            <div className="bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden">
                <div className="bg-slate-900 p-6 flex justify-between items-center text-white">
                    <h2 className="text-xl font-black tracking-tight">{initialData ? "Modify Notice" : "New Notice"}</h2>
                ...
                <div className="p-8">
                    <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); onSave(formData); }}>
                        <div className="space-y-1">
                            ...
                            <input required value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} className="w-full text-black px-4 py-3 rounded-2xl bg-slate-50 border border-slate-200 outline-none focus:ring-4 focus:ring-blue-500/10 transition-all font-bold" />
                        </div>
                        ...
                        <div className="space-y-1">
                            ...
                            <textarea required rows={4} value={formData.content} onChange={e => setFormData({ ...formData, content: e.target.value })} className="w-full text-black px-4 py-3 rounded-2xl bg-slate-50 border border-slate-200 outline-none focus:ring-4 focus:ring-blue-500/10 transition-all font-medium" />
                        </div>

                        <button type="submit" className="w-full text-white bg-slate-900  py-4 rounded-2xl font-black text-lg shadow-xl hover:bg-black transition-all mt-2 uppercase tracking-widest">
